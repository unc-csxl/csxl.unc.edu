...
def test_create_event_and_get_by_id(
    event_service: EventService, organization: OrganizationService
):
...
def test_get_from_time_range(
    ...
    event_service.create(event1)
    event_service.create(event2)
    event_from_range1 = event_service.get_from_time_range(
        datetime.datetime.fromtimestamp(1680110860),
        datetime.datetime.fromtimestamp(1680110862),
    )
    event_from_range2 = event_service.get_from_time_range(
        datetime.datetime.fromtimestamp(1880110860),
        datetime.datetime.fromtimestamp(1880110862),
    )
...
def test_update(event_service: EventService, organization: OrganizationService):
    ...
    assert (
        event_service.get_from_id(1).location
        == new_event.location
        == event1_updated.location
    )
