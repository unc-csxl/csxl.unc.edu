...
def test_get_slow_data(signage_svc: SignageService, signage_scenario: SignageScenario):
    ...
    assert all(
        article.title != signage_scenario.announcement.title
        for article in slow_data.newest_news
    )
    ...
    assert (
        slow_data.top_users[0].first_name
        == signage_scenario.reservation.ambassador.first_name
    )
    assert (
        slow_data.top_users[0].last_name
        == signage_scenario.reservation.ambassador.last_name
    )
    assert (
        slow_data.top_users[1].first_name
        == signage_scenario.reservation.root.first_name
    )
    assert (
        slow_data.top_users[1].last_name == signage_scenario.reservation.root.last_name
    )
    assert (
        slow_data.top_users[2].first_name
        == signage_scenario.reservation.user.first_name
    )
    assert (
        slow_data.top_users[2].last_name == signage_scenario.reservation.user.last_name
    )
