...
def upgrade() -> None:
    op.create_table('coworking__reservation_user',
        ...
        sa.ForeignKeyConstraint(['reservation_id'], ['coworking__reservation.id'], name='coworking__reservation_user_reservation_id_fkey'),
        sa.ForeignKeyConstraint(['user_id'], ['user.id'], name='coworking__reservation_user_user_id_fkey'),
        sa.PrimaryKeyConstraint('reservation_id', 'user_id', name='coworking__reservation_user_pkey')
    )

    op.create_table('coworking__seat',
        sa.Column('id', sa.INTEGER(), server_default=sa.text("nextval('coworking__seat_id_seq'::regclass)"), autoincrement=True, nullable=False),
        ...
        sa.ForeignKeyConstraint(['room_id'], ['coworking__room.id'], name='coworking__seat_room_id_fkey'),
        sa.PrimaryKeyConstraint('id', name='coworking__seat_pkey'),
        postgresql_ignore_search_path=False
    )

    op.create_table('coworking__room',
    ...
    op.create_index('ix_coworking__room_capacity', 'coworking__room', ['capacity'], unique=False)

    op.create_table('coworking__reservation_seat',
        sa.Column('reservation_id', sa.INTEGER(), autoincrement=False, nullable=False),
        sa.Column('seat_id', sa.INTEGER(), autoincrement=False, nullable=False),
        sa.ForeignKeyConstraint(['reservation_id'], ['coworking__reservation.id'], name='coworking__reservation_seat_reservation_id_fkey'),
        sa.ForeignKeyConstraint(['seat_id'], ['coworking__seat.id'], name='coworking__reservation_seat_seat_id_fkey'),
        sa.PrimaryKeyConstraint('reservation_id', 'seat_id', name='coworking__reservation_seat_pkey')
    )
    ...
    op.create_index('ix_coworking__operating_hours_start', 'coworking__operating_hours', ['start'], unique=False)
    op.create_index('ix_coworking__operating_hours_end', 'coworking__operating_hours', ['end'], unique=False)
    ...
    op.create_table('coworking__reservation',
        ...
        sa.ForeignKeyConstraint(['room_id'], ['coworking__room.id'], name='coworking__reservation_room_id_fkey'),
