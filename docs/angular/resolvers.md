...
You want to be cautious about using Resolvers for *everything*, though. In situations where your page needs to dynamically update, for example, using Resolvers is not the best idea (because Resolvers load your data *once*). Instead, using a convention such as the CSXL's custom `RxObject` class may be a better idea. Learn more about that [here](https://github.com/unc-csxl/csxl.unc.edu/blob/main/docs/angular/rxobject.md).
